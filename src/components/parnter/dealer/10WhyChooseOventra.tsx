...
// Maroon     #800020  — brand ink, primary action, active state
...
const pillars = [
  {
    icon: Layers,
    label: "Built for dealer satisfaction",
    body: "Dealer satisfaction directly influences distributor performance and ultimately impacts your brand's market growth.",
  },
  {
    icon: Footprints,
    label: "Self-service ordering",
    body: "OVENTRA empowers dealers with a modern, self-service ordering experience while giving distributors and manufacturers complete control over the ordering lifecycle.",
  },
  {
    icon: Radar,
    label: "One connected ecosystem",
    body: "Instead of disconnected communication channels and manual processes, OVENTRA creates a connected B2B commerce ecosystem where dealers can discover products, place orders, and track every transaction with confidence.",
  },
];
...
const WhyChooseOVENTRA = () => {
  ...
  return (
    <section className="relative overflow-hidden bg-[#efe6e8] px-4 py-20 md:px-8 md:py-28">
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl gap-12 lg:grid-cols-12 lg:gap-8">
        ...
        >
          <span className="mb-3 inline-block text-xs font-semibold uppercase tracking-[0.2em] text-[#800020]">
            Platform
          </span>
          <h2 className="text-3xl font-black leading-[1.05] tracking-tight text-[#150808] md:text-5xl">
            Why Choose <span className="text-[#800020]">OVENTRA</span>?
          </h2>
          <p className="mt-5 max-w-md text-sm leading-relaxed text-[#150808]/60 md:text-base">
            The result is a stronger channel network, increased ordering
            frequency, and improved business efficiency.
          </p>
  ...
  whileTap={{ scale: 0.97 }}
  className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#800020] px-7 py-3 text-sm font-bold text-white transition-colors hover:bg-[#600000]"
>
  Book a Demo
  <ArrowRight size={16} />
</motion.a>
                  ...
                  >
                    <span
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors duration-300 ${
                        open ? "bg-[#800020] text-white" : "bg-[#204a8c] text-white"
                      }`}
                    >
                      <p.icon size={18} />
                    </span>
                    ...
                    <span className="flex-1">
                      <span className="block text-[10px] font-semibold uppercase tracking-[0.2em] text-[#800020]/70">
                        Spec {String(i + 1).padStart(2, "0")}
                      </span>
                     <span
  className={`mt-0.5 block text-base font-bold sm:text-lg ${
    open ? "text-[#800020]" : "text-[#204a8c]"
  }`}
>
  {p.label}
</span>
