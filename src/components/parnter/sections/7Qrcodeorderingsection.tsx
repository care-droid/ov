    ...
    <section className="relative w-full py-20 sm:py-28" style={{ background: "efe6e8" }}>
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 px-6 sm:px-10 lg:grid-cols-2 lg:gap-16">
        ...
        <div className="order-1 lg:order-2">
          ...
          {/* Steps — 2 columns, 3 items each (left column: first 3, right column: last 3) */}
          <motion.ul
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.5 }}
            transition={{ duration: 0.55, delay: 0.18 }}
            className="mt-5 grid max-w-md grid-cols-2 grid-flow-col grid-rows-3 gap-x-8 gap-y-3"
          >
            {steps.map((step) => (
              <li
                key={step}
                className="flex items-center gap-2 text-[15px] leading-relaxed"
                style={{ color: `${INK}99` }}
              >
                <span
                  className="grid h-5 w-5 flex-shrink-0 place-items-center rounded-full text-[10px] font-bold text-white"
